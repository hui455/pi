 ...
 * 演示如何在不改变内置工具（read、bash、edit、write）行为的前提下覆盖其渲染方式。
 * 每个工具都以相同名称重新注册，执行委托给原始实现，
 * 同时提供紧凑的自定义 renderCall/renderResult 函数。
 *
