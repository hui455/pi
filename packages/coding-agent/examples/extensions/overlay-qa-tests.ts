 ...
 *   /overlay-sidepanel  - 响应式侧边面板（终端宽度 < 100 列时隐藏）
...
export default function (pi: ExtensionAPI) {
	...
	// 动画演示 —— 证明 overlay 能处理实时更新（pi-doom 就需要这样的能力）
	...
	// 测试 overlay 切换 —— 演示通过 onHandle 回调调用 OverlayHandle.setHidden()
	...
	// 焦点循环演示 —— 演示 focus()、输入路由、逐面板关闭和渲染顺序
...
class StreamingOverflowComponent extends BaseOverlay {
	...
	private startProcess(): void {
		...
		this.proc.stdout?.on("data", (data: Buffer) => {
			if (this.disposed) return; // Guard against callbacks after dispose（防止销毁后的回调）
...
// MaxHeight 测试 —— 渲染 20 行，被 maxHeight 截断为 10 行
...
// 动画演示 —— 证明 overlay 能像 pi-doom 一样处理实时更新
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		// 动画内容 —— 弹跳的横条
		const barWidth = Math.max(12, innerW - 4); // Ensure enough space for bar（确保横条有足够空间）
...
// 切换演示 —— 演示通过 onHandle 回调调用 OverlayHandle.setHidden()
...
class StreamingInputController extends BaseOverlay {
	...
	private focusIndex = -1; // -1 = controller focused, 0-2 = panel focused（-1 = 控制器获得焦点，0-2 = 面板获得焦点）
