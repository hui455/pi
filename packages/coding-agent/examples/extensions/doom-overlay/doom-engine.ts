...
export class DoomEngine {
	...
	getFrameRGBA(): Uint8Array {
		...
		for (let i = 0; i < pixels; i++) {
			...
			const offset = i * 4;
			buffer[offset + 0] = (argb >> 16) & 0xff; // R（红）
			buffer[offset + 1] = (argb >> 8) & 0xff; // G（绿）
			buffer[offset + 2] = argb & 0xff; // B（蓝）
			buffer[offset + 3] = 255; // A（透明度）
		}
