 ...
 * 扫描项目的 .claude/rules/ 文件夹以查找规则文件，并将其列在系统提示词中。
 * 之后代理可以在需要时使用 read 工具加载特定规则。
 *
 ...
 * - 使用描述性文件名：文件名应表明规则覆盖的内容
 * - Use conditional rules sparingly: Only add paths frontmatter when rules truly apply to specific file types
 * - 谨慎使用条件规则：只有当规则确实适用于特定文件类型时才添加 paths frontmatter
 * - Organize with subdirectories: Group related rules (e.g., frontend/, backend/)
 * - 用子目录组织：将相关规则分组（例如 frontend/、backend/）
 *
 * Usage:
 * 用法：
